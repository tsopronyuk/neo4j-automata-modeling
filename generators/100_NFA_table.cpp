...
public:
    ...
    // ----------------------------------------------------------------
    // Structural validation of a generated NFA
    //
    // This check does not modify the generated automaton or benchmark
    // metrics. It verifies the structural assumptions of each instance.
    // ----------------------------------------------------------------
    bool validate(const std::vector<std::string>& alphabet,
        ...
        if (start_count != 1) {
            error = "Expected exactly one initial state, found " +
                    std::to_string(start_count);
            return false;
        }

        // 2. At least one accepting/final state.
        ...
        if (final_count == 0) {
            error = "No accepting/final state found";
            return false;
        }

        // 3. State identifiers must be unique.
        std::set<std::string> state_ids;

        for (const auto& st : states_) {
            if (!state_ids.insert(st.id).second) {
                error = "Duplicate state identifier: " + st.id;
                return false;
            }
        }

        // 4. Every transition source and target must refer to an
        //    existing state, and every label must belong to the alphabet.
        std::set<std::string> alphabet_set(alphabet.begin(), alphabet.end());

        for (const auto& [from_id, sym_map] : delta_) {
            if (state_ids.find(from_id) == state_ids.end()) {
                error = "Transition source does not exist: " + from_id;
                return false;
            }

            for (const auto& [symbol, targets] : sym_map) {
                if (alphabet_set.find(symbol) == alphabet_set.end()) {
                    error = "Transition uses symbol outside alphabet: " +
                            symbol;
                    return false;
                }

                for (const auto& target_id : targets) {
                    if (state_ids.find(target_id) == state_ids.end()) {
                        error = "Transition target does not exist: " +
                                target_id;
                        return false;
                    }
                }
        ...
        // 5. Every state must be reachable from the initial state.
        ...
        for (size_t i = 0; i < queue.size(); ++i) {
            ...
            auto it = delta_.find(current);
            if (it == delta_.end()) {
                continue;
            }

            for (const auto& [symbol, targets] : it->second) {
        ...
        if (visited.size() != states_.size()) {
            error = "Not all states are reachable from the initial state: " +
...
int main(int argc, char* argv[]) {
    ...
    // Count benchmark instances that pass structural validation.
    size_t validated_instances = 0;

    // --- Run experiments ---
    std::cout << "Generating and validating automata...\n" << std::flush;
    ...
    for (size_t scale : target_scales) {
        ...
        for (int rep = 0; rep < num_replications; ++rep) {
            ...
            // Validate the generated NFA before collecting benchmark metrics.
            std::string validation_error;
            if (!aut.validate(alphabet, validation_error)) {
